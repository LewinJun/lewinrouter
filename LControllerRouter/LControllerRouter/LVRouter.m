...
//  LControllerRouter
//
//  Created by Li Zhiyu on 16/5/19.
//  Copyright © 2016年 Li Zhiyu. All rights reserved.
//

#import "LVRouter.h"
#import <objc/runtime.h>

#pragma mark - LVRouter_private
@interface NSString (LVRouter_private)
...
- (void)showViewController:(NSString *)viewControllerName param:(NSDictionary *)param animation:(BOOL)animation
{
    ...
    if (vc) {
        ...
        if (nvc) {
            ...
            [self __pushViewController:vc parameters:param atNavigationController:nvc animated:animation];
            
        }else
        {
            [self __pressntVController:vc parameters:param animated:animation];
        }
    }
}
...
- (void)showViewController:(NSString *)viewControllerName param:(NSDictionary *)param
{
    ...
    [self showViewController:viewControllerName param:param animation:YES];
}
...
- (id)viewControllerForKey:(NSString *)key
{
    ...
    {
        LVRouterBlock objBlock = (LVRouterBlock)obj;
        vc = objBlock();
    }
...
#pragma mark - private
- (LVRouterType)routeTypeByComponent:(NSString *)component
{
    if ([@"." isEqualToString:component])
    {
        return LVRouterURLType_push;
    }
    else if ([@".." isEqualToString:component])
    {
        return LVRouterURLType_pushAfterPop;
    }
    else if ([@"/" isEqualToString:component])
    {
        return LVRouterURLType_pushAfterGotoRoot;
    }
...
{
    LVRouterType type = [self routeTypeByComponent:[components firstObject]];
    BOOL animated = NO;
    ...
    if (type == LVRouterURLType_push)
    {
    }
    else if (type == LVRouterURLType_pushAfterPop)
    {
        [navigationController popViewControllerAnimated:animated];
    }
    else if (type == LVRouterURLType_pushAfterGotoRoot)
    {
...
#pragma mark -
static const char *LVRouter_URLPath = "XY.ViewController.URLPath";

@implementation UIViewController (LVRouter)

- (NSString *)uxy_URLPath
{
    return objc_getAssociatedObject(self, LVRouter_URLPath);
}

- (void)setUxy_URLPath:(NSString *)uxy_URLPath
{
    objc_setAssociatedObject(self, LVRouter_URLPath, uxy_URLPath, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
